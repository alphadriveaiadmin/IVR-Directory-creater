...
if st.button("Generate IVR Directory"):
    if not campaign_id.strip():
        st.error("Please enter a 4-digit campaign ID before generating.")
    elif not campaign_id.isdigit() or len(campaign_id) != 4:
        st.error("Campaign ID must be exactly 4 digits.")
    else:
        try:
            ...
            try:
                ...
                st.error("Webhook response was not valid JSON.")
